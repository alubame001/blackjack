...
*/
function GameState (board,boardOutput,gameState,gameLoop) {
   
    this.currentState= {}
    this.stateTimer = 0
    this.states={}
    this.states.waitingForPlayer={}

    this.states.waitingForPlayer.beginState= function(){
    };
    this.states.waitingForPlayer.endState= function(){
    };    
    this.states.waitingForPlayer.betRequest= function(){
    };

    this.states.waitingForPlayer.addPlayer= function(board, requestData){
          return board.addPlayer(requestData["clientID"], requestData["requestedPosition"]);
    };

    this.states.waitingForPlayer.splitRequest= function(){
    };

    this.states.waitingForPlayer.hitRequest= function(){
    };
    this.states.waitingForPlayer.standRequest= function(){
    };
    this.states.waitingForPlayer.doubleDownRequest= function(){
    };
    this.states.waitingForPlayer.hideDealerCard= function(){
    };
    this.states.waitingForPlayer.message ="Waiting for players to join."
    this.states.waitingForPlayer.wait = 1000

    this.states.acceptingBets={}
    this.states.acceptingBets.dropPlayersTimer="0"
    this.states.acceptingBets.dropPlayersTimeout=function(board,gameState){
                board.removeAllPlayers();
                board.resetBoard();
                gameLoop.resetLoop();
                gameLoop.io.sockets.emit('updateTable', boardOutput.getBoard());
